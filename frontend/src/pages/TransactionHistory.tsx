  ...
  return (
    <div>
      ...
      <div className="mb-8">
        ...
        <p className="text-sm font-mono text-dark-500 uppercase tracking-wider mt-2">Complete Transaction Log</p>
      </div>

      {/* Info Banner */}
      <div className="bg-vault-dark-4 border border-dark-600 rounded-md p-3 mb-4">
      ...
      <div className="vault-panel p-8 mb-8">
        ...
        {isLoadingHistory ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-xs font-mono text-dark-600 uppercase tracking-wider">Accessing vault records</p>
          </div>
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-dark-500 font-semibold">No transaction history</p>
            <p className="text-xs text-dark-600 mt-1 font-mono uppercase tracking-wider">
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {decoded.details && (
                        <p className="text-sm text-dark-200 font-semibold mt-2">{decoded.details}</p>
                      )}
                      <p className="text-xs font-mono text-dark-500 mt-2 uppercase tracking-wider">{formatTimestamp(tx.timestamp)}</p>
                    </div>
                    <div className="text-right ml-4 flex-shrink-0">
                      {tx.value !== '0' && (
                        <p className="text-base font-display font-bold text-gradient-red vault-text-glow">
                          ...
                          <span className="text-sm text-primary-400 ml-1">QUAI</span>
                        </p>
                      )}
                  ...
                  {tx.to.toLowerCase() !== walletAddress.toLowerCase() && (
                    <div className="bg-vault-dark-4 rounded-md p-4 mb-4 border border-dark-600 space-y-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-mono text-sm text-primary-300">{formatAddress(tx.to)}</span>
                      </div>
                      {tx.data !== '0x' && decoded.type === 'contractCall' && (
                        <div className="flex justify-between text-sm">
                          <span className="text-xs font-mono text-dark-500 uppercase tracking-wider">Data:</span>
                          <span className="font-mono text-xs text-dark-400 break-all max-w-xs text-right">
                  ...
                  {Object.keys(tx.approvals).length > 0 && (
                    <div className="vault-divider pt-4 mt-4">
                      <p className="text-xs font-mono text-dark-500 uppercase tracking-wider mb-3">Approved by:</p>
      ...
      <div className="vault-panel p-8 opacity-90">
        ...
        {isLoadingCancelled ? (
          ...
          <div className="text-center py-12">
            ...
            <p className="text-sm text-dark-500 font-semibold">No cancelled transactions</p>
            <p className="text-xs text-dark-600 mt-1 font-mono uppercase tracking-wider">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <span className={`inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold ${decoded.bgColor} ${decoded.textColor} border ${decoded.borderColor} shadow-vault-inner opacity-75`}>
                          <span className="mr-1.5">{decoded.icon}</span>
                          {decoded.description}
                        </span>
                      ...
                      {decoded.details && (
                        <p className="text-sm text-dark-300 font-semibold mt-2">{decoded.details}</p>
                      )}
                      <p className="text-xs font-mono text-dark-600 mt-2 uppercase tracking-wider">{formatTimestamp(tx.timestamp)}</p>
                    </div>
                    <div className="text-right ml-4 flex-shrink-0">
                      {tx.value !== '0' && (
                  ...
                  {tx.to.toLowerCase() !== walletAddress.toLowerCase() && (
                    <div className="bg-vault-dark-4 rounded-md p-4 mb-4 border border-dark-600 space-y-3">
                      <div className="flex justify-between text-sm">
                        <span className="text-xs font-mono text-dark-600 uppercase tracking-wider">To:</span>
                        <span className="font-mono text-sm text-dark-400">{formatAddress(tx.to)}</span>
                      </div>
                      {tx.data !== '0x' && decoded.type === 'contractCall' && (
                        <div className="flex justify-between text-sm">
                          <span className="text-xs font-mono text-dark-600 uppercase tracking-wider">Data:</span>
                          <span className="font-mono text-xs text-dark-500 break-all max-w-xs text-right">
                            {tx.data.length > 50 ? `${tx.data.slice(0, 50)}...` : tx.data}
                          </span>
                        </div>
                      )}
                  ...
                  {Object.keys(tx.approvals).length > 0 && Object.values(tx.approvals).some(v => v) && (
                    <div className="vault-divider pt-4 mt-4">
                      <p className="text-xs font-mono text-dark-600 uppercase tracking-wider mb-3">Was approved by:</p>
                      <div className="flex flex-wrap gap-2">
                        {Object.entries(tx.approvals)
                          .filter(([, approved]) => approved)
                          .map(([owner]) => (
