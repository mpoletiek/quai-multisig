...
export function WalletCreationFlow({
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <p className="text-sm font-mono text-dark-500 uppercase tracking-wider mb-8">Deploy a new secure multisig wallet</p>
      ...
      {progress.step === 'preparing' && (
        <div className="vault-panel p-8 mb-8">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-xs font-mono text-dark-500 uppercase tracking-wider mb-3">Owners</label>
              <div className="space-y-2">
                {owners.map((owner, index) => (
                  <div
                    key={index}
                    className="px-4 py-3 bg-vault-dark-4 rounded-md border border-dark-600 text-sm font-mono text-primary-300"
                  >
                    {owner}
                  </div>
                ))}
            ...
            <div>
              <label className="block text-xs font-mono text-dark-500 uppercase tracking-wider mb-3">Required Approvals</label>
              <p className="text-lg font-semibold text-dark-200">
                ...
                <span className="text-dark-500 ml-2 text-sm font-normal">owner{owners.length !== 1 ? 's' : ''}</span>
              </p>
            </div>
            ...
            <div className="flex gap-6">
              ...
              <div className="flex-1">
                ...
                {progress.deployTxHash && (
                  <div className="mt-3 bg-vault-dark-4 rounded-md p-3 border border-dark-600">
                ...
                {isStepActive('deploying') && (
                  <div className="mt-4 flex items-center gap-2 text-sm text-dark-500">
                    <div className="relative">
                      ...
                      <div className="relative w-4 h-4 border-2 border-primary-600 border-t-transparent rounded-full animate-spin"></div>
                    </div>
            ...
            {progress.step === 'success' && progress.walletAddress && (
              <div className="mt-8 p-6 bg-gradient-to-br from-primary-900/30 via-primary-800/20 to-primary-900/30 border-2 border-primary-600/50 rounded-md shadow-red-glow">
                ...
                <div className="bg-vault-dark-4 rounded-md p-4 border border-dark-600">
                  <p className="text-xs font-mono text-dark-500 uppercase tracking-wider mb-3">Vault Address</p>
                  <button
                    onClick={async () => {
                      try {
                  ...
                  >
                    <div className="flex items-center gap-3 bg-vault-dark-3 px-4 py-3 rounded-md border border-dark-600 hover:border-primary-600/50 hover:bg-vault-dark-2 transition-all">
                      <code className="text-sm font-mono text-primary-300 break-all flex-1 group-hover:text-primary-200 transition-colors select-all">
                      ...
                      <div className={`flex-shrink-0 transition-all duration-200 ${copied ? 'opacity-100' : 'opacity-40 group-hover:opacity-100'}`}>
                        {copied ? (
                          ...
                          <svg className="w-4 h-4 text-primary-500 group-hover:text-primary-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {copied && (
                      <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 bg-primary-900 text-primary-200 px-3 py-1.5 rounded-md text-xs font-semibold border border-primary-700 shadow-red-glow z-10 animate-in fade-in duration-200">
                        Address copied!
